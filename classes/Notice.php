...
class Notice extends Managed_DataObject
{
    ...
    public $reposts_count;                   //int
    public $comments_count;                  //int
    public $favorites_count;                  //int
    ...
    public static function schemaDef()
    {
        $def = array(
            'fields' => array(
                ...
                'reposts_count' => array('type' => 'int', 'description' => 'reposts count'),
                'comments_count' => array('type' => 'int', 'description' => 'comments count'),
                'favorites_count' => array('type' => 'int', 'description' => 'favorites_count'),
                
                
                'content_type' => array('type' => 'int', 'description' => 'notice this is a common post or mentions or comment'),
            ...
            'indexes' => array(
                ...
                'notice_content_type_idx' => array('content_type')
            )
        );
    ...
    const CONTENT_TYPE_MENTIONS     = 2;
    const CONTENT_TYPE_REPEAT       = 4;
    const CONTENT_TYPE_POST         = 8;
    const CONTENT_TYPE_ACTIVITY     = 16;
    ...
    function saveTags()
    {
        ...
        $count = preg_match_all('/\#([^\#|.]+)\#/', strtolower($this->content), $match);
        if (!$count) {
            return true;
        }
    ...
    static function saveNew($profile_id, $content, $source, $options=null) {
        ...
        $notice->content_type = NOTICE::CONTENT_TYPE_POST; // default value
    ...
    function addRepostCount($notice_id)
    {   
        $notice = new Notice();
        $notice->id = $notice_id;
        $notice->find();
        ...
        {
            $orig = clone($notice);
            if (empty($notice->reposts_count))
            {
                $notice->reposts_count = 1;
            }
            else
            {
                $notice->reposts_count++;
            }
            ...
            Notice::blowRepostsCount($notice_id);
        }
    }
    
    function reduceRepostCount($notice_id)
    {
        $notice = new Notice();
        $notice->id = $notice_id;
        $notice->find();
        ...
        {
            $orig = clone($notice);
            if (empty($notice->reposts_count) || $notice->reposts_count == 0)
            {
                $notice->reposts_count = 0;
            }
            else
            {
                $notice->reposts_count--;
    ...
    function addCommentCount($notice_id)
    {
        $notice = new Notice();
        $notice->id = $notice_id;
        $notice->find();
        ...
        {
            $orig = clone($notice);
            if (empty($notice->comments_count))
            {
                $notice->comments_count = 1;
            }
            else
            {
            ...
            Notice::blowCommentCount($notice_id);
        }
    }
    
    function reduceCommentCount($notice_id)
    {
        $notice = new Notice();
        $notice->id = $notice_id;
        $notice->find();
        ...
        {
            $orig = clone($notice);
            if (empty($notice->comments_count) || $notice->comments_count == 0)
            {
    ...
    function addFavoriteCount($notice_id)
    {
        $notice = new Notice();
        $notice->id = $notice_id;
        $notice->find();
        ...
        {
            $orig = clone($notice);
            if (empty($notice->favorites_count))
            {
                $notice->favorites_count = 1;
            }
            else
            {
                $notice->favorites_count++;
            }
            ...
            Notice::blowFavesCount($notice_id);
        }
    }
    
    function reduceFavoriteCount($notice_id)
    {
        $notice = new Notice();
        $notice->id = $notice_id;
        $notice->find();
        ...
        {
            $orig = clone($notice);
            if (empty($notice->favorites_count) || $notice->favorites_count == 0)
            {
                $notice->favorites_count = 0;
            }
            else
            {
                $notice->favorites_count--;
    ...
    function saveReplies()
    {
        ...
        foreach ($mentions as $mention) {
            ...
            foreach ($mention['mentioned'] as $mentioned) {
                ...
                $this->saveReply($mentioned->id, TRUE);
    ...
    function repeat($repeater_id, $source)
    {
        ...
        $content = sprintf(_('REPOST @%1$s %2$s'),
                           $author->nickname,
                           $this->content);
    ...
    function clearReplies()
    {
        ...
        if (!empty($this->reply_to))
        {
            $replyToNotice = new Notice();
            $replyToNotice->id = $this->reply_to;
            
            if ($replyToNotice->find())
            {
                $this->reduceCommentCount($replyToNotice->id);
            }
        }
    }
    ...
    static function blowCommentCount($notice_id)
    {
        self::blow('notice:comments_count:'.$notice_id);
    }
    
    static function blowRepostsCount($notice_id)
    {
        self::blow('notice:reposts_count:'.$notice_id);
    }
    
    static function blowFavesCount($notice_id)
    {
        self::blow('notice:favorites_count:'.$notice_id);
    }
    
    static function getCommentCount($notice_id)
    {
        $c = Cache::instance();
        if (!empty($c)) {
            $cnt = $c->get(Cache::key('notice:comments_count:'.$notice_id));
            if (is_integer($cnt)) {
                return (int) $cnt;
            }
        }
        ...
        $cnt = $original->comments_count;
        
        if (!empty($c)) {
            $c->set(Cache::key('notice:comments_count:'.$notice_id), $cnt);
        }

        return empty($cnt) ? 0 : $cnt;
    }
    
    static function getReposts_count($notice_id)
    {
        $c = Cache::instance();
        if (!empty($c)) {
            $cnt = $c->get(Cache::key('notice:reposts_count:'.$notice_id));
            if (is_integer($cnt)) {
                return (int) $cnt;
            }
        }
        ...
        $cnt = $original->reposts_count;
        
        if (!empty($c)) {
            $c->set(Cache::key('notice:reposts_count:'.$notice_id), $cnt);
        }
