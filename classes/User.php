...
class User extends Managed_DataObject
{
    ...
    const PLATFORM_TYPE_IKNOW = 0;               //iknow normal user
    const PLATFORM_TYPE_SINA  = 1;               //user from sina weibo
    const PLATFORM_TYPE_TENCENT_QQ  = 2;         //user from tencent qq
    ...
    public $platform_userid;                 // BIGINT     
    public $platform_type;                   // tinyint
    public $plan_password;                   // varchar(255)
    ...
    public static function schemaDef()
    {
        return array(
            ...
            'fields' => array(
                ...
                'plan_password' => array('type' => 'varchar', 'length' => 255, 'description' => 'plan password of password'),
                ...
                'platform_userid' => array('type' => 'bigint', 'default' => 0, 'description' => 'platform userid'),
                'platform_type'=> array('type' => 'int', 'size '=> 'tiny', 'default '=> 0,'description' => 'platform type identify sina tencent.etc'),
    ...
    static function register($fields) {
        ...
        $user->platform_type = 0;
        $user->platform_userid = 0;
        $user->plan_password = $password;
        ...
        if (Event::handle('StartUserRegister', array(&$user, &$profile))) {
            ...
            if (!empty($platform_userid))
            {
                $user->platform_userid = $platform_userid;
            }
    ...
    static function getSinaUsername($userId)
    {
        $user = $this->trimmed($userId);
        return 'sinaweibo'.$user;
    }
    
    static function getOtherPartformDefaultPassword()
    {
        return '123456';
    }
}
